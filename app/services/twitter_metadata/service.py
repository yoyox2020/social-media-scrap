"""Data lengkap post Twitter/X utk dipakai FRONTEND (2026-07-24) -- pola
SAMA PERSIS dgn threads_metadata/service.py, reuse tabel posts/comments
(platform="twitter"). `quotes` field tambahan (khas Twitter/Threads,
retweet quote != reply biasa) -- Twitter, BEDA dari Facebook/Instagram/
Threads, juga py `views` NYATA (bukan selalu 0)."""
...
SortKey = Literal["trend_score", "engagement_score", "freshness_score", "authority_score", "likes", "views", "published_at"]


def _sort_column(sort_by: str):
    if sort_by in ("likes", "views"):
        return cast(Post.metrics[sort_by].astext, Float)
...
async def list_posts(
    ...
    base_filters = [Post.platform == "twitter"]
    if topic:
        base_filters.append(
            or_(
...
async def get_post_detail(db: AsyncSession, post_id: str) -> dict | None:
    post = await db.scalar(select(Post).where(Post.platform == "twitter", Post.external_id == post_id))
    if not post:
        return None
...
async def list_topics(db: AsyncSession) -> list[dict]:
    ...
    rows = (await db.scalars(select(Post.metadata_).where(Post.platform == "twitter"))).all()

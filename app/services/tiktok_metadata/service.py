"""Data lengkap post TikTok utk dipakai FRONTEND (2026-07-23) -- pola
SAMA PERSIS dgn app/services/youtube_metadata/service.py, reuse tabel
`posts`/`comments` yg sama (platform="tiktok"), TIDAK bikin tabel baru.
Publik (tanpa login), sama spt YouTube.

Beda field dari YouTube: TikTok tidak py "channel_id"/subscriber count
terpisah -- pakai `author_fans` (dari metadata_) sbg gantinya."""
...
def _post_to_dict(p: Post, comment_count: int) -> dict:
    ...
    return {
        ...
        "content": p.content,
        "author": p.author,
        "author_fans": meta.get("author_fans"),
        "audience_size": meta.get("audience_size") or meta.get("author_fans"),
        "url": p.url,
        "thumbnail": thumb,
        "metrics": {
...
async def list_posts(
    ...
    base_filters = [Post.platform == "tiktok"]
    if topic:
        base_filters.append(
            or_(
    ...
    if search:
        like = f"%{search}%"
        base_filters.append(or_(Post.title.ilike(like), Post.author.ilike(like), Post.content.ilike(like)))
...
async def get_post_detail(db: AsyncSession, video_id: str) -> dict | None:
    post = await db.scalar(select(Post).where(Post.platform == "tiktok", Post.external_id == video_id))
    if not post:
        return None
...
async def list_topics(db: AsyncSession) -> list[dict]:
    """Sama pola dgn youtube_metadata: agregasi di Python (Postgres
    menolak GROUP BY atas ekspresi JSON yg SQLAlchemy bind terpisah)."""
    from collections import Counter

    rows = (await db.scalars(select(Post.metadata_).where(Post.platform == "tiktok"))).all()

"""Data lengkap post Instagram utk dipakai FRONTEND (2026-07-24) --
pola SAMA PERSIS dgn facebook_metadata/service.py, reuse tabel
posts/comments (platform="instagram"), TIDAK bikin tabel baru. Publik
(tanpa login).

Beda dari Facebook (dicek LANGSUNG ke 106 post Instagram yg SUDAH ADA
di DB sebelum file ini ditulis): `media`/thumbnail SUDAH terisi (96/106,
beda dari Facebook yg 0), `title` tetap selalu kosong, `author_followers`
dari metadata_.followers (SAMA nama field dgn Facebook, TAPI 0/106 post
py isinya sampai backfill dibangun -- lihat
app/agents/instagram/metadata_backfill.py)."""
...
async def list_posts(
    ...
    base_filters = [Post.platform == "instagram"]
    if topic:
        base_filters.append(
            or_(
...
async def get_post_detail(db: AsyncSession, post_id: str) -> dict | None:
    post = await db.scalar(select(Post).where(Post.platform == "instagram", Post.external_id == post_id))
    if not post:
        return None
...
async def list_topics(db: AsyncSession) -> list[dict]:
    ...
    rows = (await db.scalars(select(Post.metadata_).where(Post.platform == "instagram"))).all()

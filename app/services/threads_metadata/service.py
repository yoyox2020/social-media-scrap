"""Data lengkap post Threads utk dipakai FRONTEND (2026-07-24) -- pola
SAMA PERSIS dgn facebook_metadata/service.py, reuse tabel posts/comments
(platform="threads"). `quotes` field TAMBAHAN (khas Threads, tidak ada
di platform lain -- reposts/quotes berbeda konsep)."""
...
async def list_posts(
    ...
    base_filters = [Post.platform == "threads"]
    if topic:
        base_filters.append(
            or_(
...
async def get_post_detail(db: AsyncSession, post_id: str) -> dict | None:
    post = await db.scalar(select(Post).where(Post.platform == "threads", Post.external_id == post_id))
    if not post:
        return None
...
async def list_topics(db: AsyncSession) -> list[dict]:
    ...
    rows = (await db.scalars(select(Post.metadata_).where(Post.platform == "threads"))).all()

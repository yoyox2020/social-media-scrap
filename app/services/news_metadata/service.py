"""Data lengkap artikel News utk dipakai FRONTEND (2026-07-24) -- pola
SAMA dgn platform lain, reuse tabel posts (platform="news"). BEDA:
artikel TIDAK punya thread komentar (lihat struktur_data.py), jadi
`comments`/`saved_comment_count` SELALU kosong -- bukan bug, memang
tidak ada mekanisme komentar publik utk News."""
...
SortKey = Literal["trend_score", "freshness_score", "authority_score", "published_at"]


def _sort_column(sort_by: str):
    if sort_by == "published_at":
        return Post.published_at
    if sort_by in ("trend_score", "freshness_score", "authority_score"):
...
async def list_posts(
    ...
    base_filters = [Post.platform == "news"]
    if topic:
        base_filters.append(
            or_(
    ...
    if search:
        like = f"%{search}%"
        base_filters.append(or_(Post.content.ilike(like), Post.title.ilike(like), Post.author.ilike(like)))
...
async def get_post_detail(db: AsyncSession, post_id: str) -> dict | None:
    post = await db.scalar(select(Post).where(Post.platform == "news", Post.external_id == post_id))
    if not post:
        return None
...
async def list_topics(db: AsyncSession) -> list[dict]:
    ...
    rows = (await db.scalars(select(Post.metadata_).where(Post.platform == "news"))).all()

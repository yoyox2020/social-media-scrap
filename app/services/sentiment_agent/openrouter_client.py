"""
Client klasifikasi sentiment via LLM (OpenRouter) -- opini KEDUA utk kasus
yg lexicon (rule-based, khusus Bahasa Indonesia) kemungkinan besar salah:
komentar BUKAN Bahasa Indonesia, atau lexicon bilang "netral" (kasus paling
sering salah krn slang tidak dikenali kamus). Reuse pola JSON-parsing dari
app/services/youtube_discovery/openrouter_client.py.
"""
...
from app.services.sentiment_agent.config import OPENROUTER_BASE_URL
...
class SentimentRateLimitError(Exception):
    ...
    tandai sudah dicek spt biasa (hindari retry selamanya). Ditemukan
    2026-07-18: tanpa pembedaan ini, komentar 'terbakar' (dilewati permanen)
    tiap kali limit harian 50-request OpenRouter habis."""
...
async def classify_sentiment(
    ...
    *,
    ...
    """
    prompt = (
        "Kamu menilai SENTIMEN satu komentar dari media sosial (YouTube). "

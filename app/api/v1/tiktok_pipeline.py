"""Trigger pipeline multi-agent TikTok (2026-07-23). Admin-only.
Lihat app/agents/pipeline.py::run_tiktok_pipeline utk alur lengkap."""
import uuid
...
router = APIRouter(prefix="/pipeline/tiktok", tags=["pipeline-tiktok"])
...
async def run_pipeline(
    ...
    agent_topic -> agent_search -> agent_tiktok (+child) ->
    agent-struktur-data -> simpan DB."""
    result = await pipeline.run_tiktok_pipeline(db, body.topic)

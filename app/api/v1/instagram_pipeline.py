"""Trigger pipeline multi-agent Instagram (2026-07-24). Admin-only.
Lihat app/agents/pipeline.py::run_instagram_pipeline utk alur lengkap."""
import uuid
...
router = APIRouter(prefix="/pipeline/instagram", tags=["pipeline-instagram"])
...
async def run_pipeline(
    ...
    agent_topic -> agent_search -> agent_instagram -> agent-struktur-data
    -> simpan DB."""
    result = await pipeline.run_instagram_pipeline(db, body.topic)

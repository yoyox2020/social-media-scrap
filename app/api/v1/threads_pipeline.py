"""Trigger pipeline multi-agent Threads (2026-07-24). Admin-only.
Lihat app/agents/pipeline.py::run_threads_pipeline utk alur lengkap."""
import uuid
...
router = APIRouter(prefix="/pipeline/threads", tags=["pipeline-threads"])
...
async def run_pipeline(
    ...
    agent_topic -> agent_search -> agent_threads -> agent-struktur-data
    -> simpan DB."""
    result = await pipeline.run_threads_pipeline(db, body.topic)

"""Data lengkap post Instagram utk FRONTEND (2026-07-24) -- PUBLIK
(tanpa login), pola SAMA PERSIS dgn facebook_metadata.py/youtube_metadata.py.
Lihat app/services/instagram_metadata/service.py utk detail field."""
from fastapi import APIRouter, Depends, Query
from sqlalchemy.ext.asyncio import AsyncSession

from app.infrastructure.database.connection import get_db
from app.services.instagram_metadata import service
from app.shared.exceptions import NotFoundError
from app.shared.utils import build_success_response

router = APIRouter(prefix="/instagram/metadata", tags=["instagram-metadata"])


@router.get("", response_model=dict)
async def list_instagram_metadata(
    ...
    """Daftar post Instagram (isi, metrics, akun, thumbnail, dll) siap
    dipakai frontend langsung."""
...
async def get_instagram_metadata_detail(post_id: str, db: AsyncSession = Depends(get_db)):
    """Detail 1 post LENGKAP + semua komentar tersimpan."""
    data = await service.get_post_detail(db, post_id)
    if not data:
        raise NotFoundError(f"Post Instagram '{post_id}' tidak ditemukan di database")

"""Data lengkap post YouTube utk FRONTEND (2026-07-22) -- PUBLIK
(tanpa login), sama pola dgn /youtube/trending-public. Lihat
app/services/youtube_metadata/service.py."""
from fastapi import APIRouter, Depends, Query
from sqlalchemy.ext.asyncio import AsyncSession

from app.infrastructure.database.connection import get_db
from app.services.youtube_metadata import service
...
async def list_youtube_metadata(
    topic: str | None = Query(default=None, description="Filter by topik pencarian (mis. 'jampidsus')"),
    search: str | None = Query(default=None, description="Cari di judul/nama channel"),
    ...
    """Daftar post YouTube (judul, metrics, skor, AI summary, dll) --
...
async def get_youtube_metadata_detail(video_id: str, db: AsyncSession = Depends(get_db)):
    """Detail 1 video LENGKAP + semua komentar tersimpan."""
    data = await service.get_post_detail(db, video_id)
    if not data:
        raise NotFoundError(f"Video YouTube '{video_id}' tidak ditemukan di database")

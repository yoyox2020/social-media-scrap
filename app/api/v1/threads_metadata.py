"""Data lengkap post Threads utk FRONTEND (2026-07-24) -- PUBLIK (tanpa
login), pola SAMA PERSIS dgn facebook_metadata.py/instagram_metadata.py.
Lihat app/services/threads_metadata/service.py utk detail field."""
from fastapi import APIRouter, Depends, Query
from sqlalchemy.ext.asyncio import AsyncSession

from app.infrastructure.database.connection import get_db
from app.services.threads_metadata import service
from app.shared.exceptions import NotFoundError
from app.shared.utils import build_success_response

router = APIRouter(prefix="/threads/metadata", tags=["threads-metadata"])


@router.get("", response_model=dict)
async def list_threads_metadata(
    ...
    """Daftar post Threads siap dipakai frontend langsung."""
...
async def get_threads_metadata_detail(post_id: str, db: AsyncSession = Depends(get_db)):
    """Detail 1 post LENGKAP + semua komentar tersimpan."""
    data = await service.get_post_detail(db, post_id)
    if not data:
        raise NotFoundError(f"Post Threads '{post_id}' tidak ditemukan di database")

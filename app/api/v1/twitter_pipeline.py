"""Trigger pipeline multi-agent Twitter/X (2026-07-24). Admin-only.
Lihat app/agents/pipeline.py::run_twitter_pipeline utk alur lengkap."""
import uuid
...
router = APIRouter(prefix="/pipeline/twitter", tags=["pipeline-twitter"])
...
async def run_pipeline(
    ...
    agent_topic -> agent_search -> agent_twitter -> agent-struktur-data
    -> simpan DB."""
    result = await pipeline.run_twitter_pipeline(db, body.topic)

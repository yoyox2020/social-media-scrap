"""Trigger pipeline multi-agent Facebook (2026-07-24). Admin-only.
Lihat app/agents/pipeline.py::run_facebook_pipeline utk alur lengkap."""
import uuid
...
router = APIRouter(prefix="/pipeline/facebook", tags=["pipeline-facebook"])
...
async def run_pipeline(
    ...
    agent_topic -> agent_search -> agent_facebook (+child) ->
    agent-struktur-data -> simpan DB."""
    result = await pipeline.run_facebook_pipeline(db, body.topic)

"""agent_threads (coordinator/parent) -- distribusi keyword ke child
aktif, pola SAMA dgn TikTok/Facebook (BUKAN Instagram yg username-only)
krn EnsembleData /threads/keyword/search genuinely keyword-search."""
...
from app.agents.threads.crawler_client import fetch_via_keywords
from app.services.agent_registry.service import get_enabled_children

AGENT_NAME = "agent_threads"
...
async def run_children(db: AsyncSession, run_id: uuid.UUID, keywords: list[dict]) -> dict:
    ...
    if not children:
        await log_activity(
            ...
            "Tidak ada child agent_threads yg aktif -- pipeline berhenti", level="warning",
        )
        return {"posts": []}

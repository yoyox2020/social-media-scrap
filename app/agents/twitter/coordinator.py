"""agent_twitter (coordinator/parent) -- distribusi keyword ke child
aktif, pola SAMA dgn Threads (genuinely keyword-search via Apify)."""
...
from app.agents.twitter.crawler_client import fetch_via_keywords
from app.services.agent_registry.service import get_enabled_children

AGENT_NAME = "agent_twitter"
...
async def run_children(db: AsyncSession, run_id: uuid.UUID, keywords: list[dict]) -> dict:
    ...
    if not children:
        await log_activity(
            ...
            "Tidak ada child agent_twitter yg aktif -- pipeline berhenti", level="warning",
        )
        return {"posts": []}

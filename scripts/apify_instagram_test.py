"""
Test scraping Instagram (post + komentar + sentiment) via Apify Actor
...
Catatan penting (lihat docs/apify-instagram-method.md untuk detail):
- latestComments HARUS > 0. Kalau 0, Actor tidak menghasilkan baris output
  sama sekali walau post berhasil di-fetch.
- Field opsional (dateFrom, dateTo, facebookProfileName, dst) jangan diisi
...
def run_instagram_test(username: str, latest_posts: int = 5, latest_comments: int = 3) -> list[dict]:
...
def main() -> None:
    ...
    items = run_instagram_test(username, latest_posts, latest_comments)

    unique_posts = {i["postUrl"] for i in items}
    print(f"\nTotal item: {len(items)} | post unik: {len(unique_posts)}")

    out_path = f"apify_instagram_{username}.json"
